...
OUTFIT_RENDER_MODEL = os.getenv("OUTFIT_RENDER_MODEL", "qwen/qwen-image-3-pro").strip()
...
OUTFIT_RENDER_TIMEOUT_SECONDS = float(os.getenv("OUTFIT_RENDER_TIMEOUT_SECONDS", "180"))
...
# 이미지 생성 reliable queue·결과 cache·SSE. 큐에는 작업 UUID만 저장하고
# 참조 이미지와 결과 메타데이터는 PostgreSQL/S3에서 읽는다.
...
OUTFIT_RENDER_QUEUE_MAX_RETRIES = int(os.getenv("OUTFIT_RENDER_QUEUE_MAX_RETRIES", "3"))
...
# 오늘의 룩은 추천 생성 워커에서 공통 렌더 서비스를 직접 호출한다. 채팅 이미지
# 작업 큐와 생명주기가 달라 별도 큐를 쓰되 결과 S3·렌더 캐시는 공유한다.
...
DAILY_LOOK_QUEUE_DEAD_KEY = os.getenv("DAILY_LOOK_QUEUE_DEAD_KEY", "daily-look:dead")
DAILY_LOOK_QUEUE_RETRY_KEY = os.getenv("DAILY_LOOK_QUEUE_RETRY_KEY", "daily-look:retry")
