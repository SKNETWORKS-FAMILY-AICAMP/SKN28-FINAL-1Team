...
### 2.7 메인 옷장 상단 레이아웃 수직 간격 균등화 (Spacing Equalization)
- 카테고리 필터(`전체`, `상의`, `하의` 등) ➔ 공유 옷장 방 목록 가로 스크롤 ➔ 초대/참여코드 입력 행 간의 수직 갭(Gap A, Gap B)을 8px로 균일하게 통일하여 PC 웹 및 모바일 환경 모두에서 한층 안정적이고 균형 잡힌 뷰포트를 구현했습니다.
