...
> **공유 옷장 종합 설계 명세서**: [SHARED_WARDROBE_SPECIFICATION.md](./SHARED_WARDROBE_SPECIFICATION.md)  
...
### 4.6 초대 링크 진입 시 미로그인 직행 & 로그인 시 초대방 자동 직통 진입 UX

- **요구사항 / 문제점**:
  - 기존에는 초대 링크(`invite?code=XXXXXX`)로 접근 시 미로그인 사용자가 초대 카드 및 안내 텍스트를 거쳐서 수동으로 로그인 버튼을 눌러야 했습니다.
  - 로그인 완료 후에도 초대장 화면에서 한번 더 수락 버튼을 눌러야 방에 진입하는 다소 지연되는 단계가 존재했습니다.
- **해결책**:
  - `invite.tsx` 진입 시 **미로그인 상태 (`!isLoggedIn`)**: `router.replace('/login?redirect=...')`로 **로그인 화면으로 즉시 직행**시켰습니다.
  - 로그인 성공 후: 원래 초대 링크 경로(`/invite?code=XXXXXX`)로 자동 복귀합니다.
  - **로그인 상태 (`isLoggedIn`)**: `joinSharedRoom(inviteCode)`를 자동 수행한 뒤 별도의 클릭 대기 없이 **해당 초대받은 공유 옷장 방(`/(tabs)/closet?tab=shared&room=roomId`)으로 즉통 직행**하도록 UX를 완성했습니다.

---

### 4.7 메인 옷장 상단 레이아웃 수직 간격 대칭 균등화 (12px Equal Spacing)

- **문제점**:
  - `SearchFilterBar` 카테고리 칩 하단에 들어 있던 `paddingBottom: 20px`로 인해, 1번째 간격(필터 ➔ 방 목록)과 2번째 간격(방 목록 ➔ 초대/참여코드 행) 간격에 눈에 띄는 픽셀 차이가 존재했습니다.
- **해결책**:
  - `search-filter-bar.tsx` (`chipRow` `paddingBottom: 6px`), `closet.tsx` (`roomTabsScrollContent` `marginTop: 6px`, `marginBottom: 12px`), `shared-space-flow.tsx` (`membersRow` `marginBottom: 12px`) 수치를 정밀 튜닝하여 **상단 1번째 갭(12px) = 하단 2번째 갭(12px) 1:1 완전 대칭 수치**로 일치시켰습니다.
