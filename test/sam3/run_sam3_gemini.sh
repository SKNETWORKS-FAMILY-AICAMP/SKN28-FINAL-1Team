...
#
# 준비물:
#   1) sam3.pt        — HF gated 가중치. https://huggingface.co 에서 액세스 승인 후
#                       다운로드하여 이 스크립트와 같은 디렉터리(test/)에 두거나
#                       SAM3_WEIGHTS 환경변수로 경로 지정.
...
if [[ ! -f "$WEIGHTS" ]]; then
    ...
    echo "HF에서 액세스 승인 후 sam3.pt를 받아 test/에 두거나 SAM3_WEIGHTS로 지정하세요." >&2
    exit 1
fi
