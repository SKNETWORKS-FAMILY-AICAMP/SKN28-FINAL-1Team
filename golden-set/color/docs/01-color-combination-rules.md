...
- **작성일**: 2026-08-06
- **판단 원천**: [`rules/color_rules.json`](../rules/color_rules.json) — 이 문서의 숫자는 전부 거기서 인용한 것이다
- **아이템 태그 연결**: [`rules/color_taxonomy_map.json`](../rules/color_taxonomy_map.json)
- **생성 스크립트**: [`tools/compute_color_attributes.py`](../tools/compute_color_attributes.py) → [`tools/build_color_rules.py`](../tools/build_color_rules.py)
- **v1(한국어 17색)**: [`rules/v1_backup/`](../rules/_archive/) 에 보존

---

## 1. 왜 표를 손으로 안 적었나

색 조합표는 136쌍이다. 손으로 적으면 두 가지가 반드시 일어난다.

1. **모순** — "A+B 기피"라고 써놓고 다른 줄에서 "B+A 허용"이 나온다.
2. **설명 불가** — "왜 이게 기피인가"에 "원래 그렇다" 말고 답할 게 없다.

그래서 **색 속성과 규칙 6개만 정의하고 등급 136쌍은 전부 계산**했다.
...
| | v1 (구) | v2 (현행) |
|---|---|---|
| 색 이름 | 한국어 17색 (`taxonomy.py::COLORS`) | 영문 17색 |
| 출처 | 옷장·상품 태그 체계 | Pinterest 코디 색조합 레퍼런스 |
| 색 공간 | HSL + RGB chroma | **CIELCh** (sRGB → XYZ(D65) → CIELAB → LCh) |
| 등급 | 권장 80 / 허용 31 / 주의 23 / 기피 2 | 권장 112 / 허용 7 / 주의 16 / 기피 1 |
...
- **시각 효과**: 명도 기준 — [체형 규칙](../../body/docs/02-body-proportion-rules.md)이 이 값을 참조한다
  - **확장**: White, Beige, Cream, Light Blue, Lavender, Blush Pink
  - **축소**: Black, Charcoal, Navy, Olive, Brown
  - **중립**: Gray, Burgundy, Rust, Mustard, Forest Green, Teal
...
136쌍(고유): **권장 112 · 허용 7 · 주의 16 · 기피 1**

> JSON의 `pair_grades`는 양방향 272건이다(`A|B`와 `B|A`를 모두 담아 조회를 단순화).
> 고유 조합 수는 그 절반인 136이다.
...
기피가 1개뿐인 것은 규칙이 느슨해서가 아니라, **v2 팔레트가 이미 톤다운된 실용 색 체계**이기 때문이다.
chroma ≥45인 색이 5개(Burgundy·Rust·Mustard·Forest Green·Lavender)뿐이고,
그중 보색 관계까지 성립하는 쌍이 하나다.

### 주의(△) — 소면적으로 쓰거나 뉴트럴로 완충 (16쌍)
...
| **웜·쿨 혼합** `[R6]` | Burgundy+Forest Green, Burgundy+Lavender, Mustard+Teal, Forest Green+Mustard, Lavender+Mustard, Light Blue+Mustard, Rust+Teal, Forest Green+Rust, Lavender+Rust, Blush Pink+Forest Green, Blush Pink+Lavender, Blush Pink+Light Blue | 사이에 White/Gray/Black을 넣어 분리 |

전체 136쌍의 등급은 `rules/color_rules.json`의 `matrix` 키에 있다.
...
v2 등급을 조회하려면 [`rules/color_taxonomy_map.json`](../rules/color_taxonomy_map.json)을 거친다.
...
| 퍼플 | Lavender | lossy | **`visual_effect`가 중립 → 확장으로 뒤집힌다.** 체형 규칙(02번)의 축소·확장 판단에 영향 |
| 블루 | Navy | **unrepresented** | v2에 중간 명도 파랑 accent가 없다. Navy는 `neutral`이라 **accent가 앵커로 바뀐다**. 데님이 여기 해당해 빈도가 높다 |
...
1. **v2 팔레트에 `Blue` 추가** — §6의 `블루` fallback 제거용. 가장 시급하다(데님 빈도).
