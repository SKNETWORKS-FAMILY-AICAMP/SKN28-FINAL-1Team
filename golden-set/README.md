...
├── color/                              # ■ 색상
│   ├── rules/                          #   판단 원천
│   │   ├── color_rules.json            #     [생성물] v2 17색(CIELCh) + 136쌍 등급
...
v2 색 이름(영문 17색)은 레퍼런스에서 온 것이고, 실제 아이템에 붙는 `color` 태그는
...
매핑 6건은 손실이 있으며(특히 `블루`·`멀티`), 상세는 [01번 문서 §6](docs/01-color-combination-rules.md)에 있다.
...
| 규칙 축 | 상태 | 근거 |
|---|---|---|
| 색상 조합 (v2 17색, 136쌍) | ✅ 완료 | CIELCh 속성 + 규칙 R1~R6 자동 등급 — 권장 112 / 허용 7 / 주의 16 / 기피 1 |
| 아이템 태그 ↔ v2 매핑 | ⚠️ **손실 6건** | `블루`·`멀티`는 v2에 대응 색 없음 — [01번 §6](docs/01-color-combination-rules.md) |
| 레퍼런스 등급표 교차검증 | ❌ **불가** | 이미지 셀 검출 실패 (7개 행 뭉개짐, na 27칸) — 계산 등급 유지 |
| 체형 분류 (가로축 5분류) | ✅ 완료 | 사이즈코리아 실측 181명 성별 백분위 |
| 체형별 핏·기장 권장 | ✅ 초안 | 수기 규칙 (`body_fit_rules.json` v0.2.0) — `should` 전용 |
...
1. **v2 팔레트에 `Blue` 추가** — 데님이 `Navy`(neutral)로 잘못 분류되는 문제. 빈도가 가장 높다.
