...
- **변경 사항**: v2의 A~F를 폐기했다. A~F는 체형이 아니라 **사이즈 사다리**였다. v3는 사이즈코리아가 실제로 쓰는 분류법(Rasband 1994 상반신 체형)을 따른다.
...
v2의 A~F는 여성 기준 가슴 76→96cm, 허리 60→80cm, 엉덩이 84→104cm를 4cm씩 등간격으로 나눈 것이다. 세 둘레가 **함께** 커지기만 하므로 이건 44/55/66/77/88/99 호칭과 같은 축, 즉 **사이즈**다.

문제는 사이즈로는 처방이 갈리지 않는다는 점이다.

> 키 165cm·몸무게 60kg인 두 사람 A와 B가 있다. A는 어깨가 넓고 엉덩이가 좁다. B는 어깨가 좁고 허리가 굵다.
> v2는 둘 다 같은 칸(D형)에 넣고 같은 옷을 권한다. 그런데 A에게 필요한 건 와이드 하의이고, B에게 필요한 건 세로선이다. **정반대 처방이다.**

실제로 `body_fit_rules.json`은 v2 문서가 A~F를 선언한 뒤에도 끝까지 5체형(역삼각/삼각/모래시계/직사각/라운드) 키를 그대로 유지했다. 규칙 파일이 문서를 따라가지 않은 것 자체가 A~F가 처방을 만들지 못한다는 증거다. v3는 새로 만드는 게 아니라 **규칙 파일이 이미 하고 있던 일에 사이즈코리아의 이름을 붙이고 경계를 다시 그은 것**이다.

> 사이즈가 필요 없다는 뜻은 아니다. 호칭·XS~XXL은 **별도 축**(`size_grade`)으로 두면 된다. 체형 값에 섞지만 않으면 된다.
...
네 축은 모두 **독립**이다. 같은 역삼각체형이라도 상체가 길면 크롭 상의를, 짧으면 롱 상의를 입는다. 목이 짧으면 그 위에 다시 V넥 제약이 붙는다. 최종 권장은 각 축의 권장을 **교집합**으로 합치고, 어느 한 축이라도 기피면 기피로 처리한다.
...
"어깨/엉덩이 0.42면 넓은 것"이라고 단정할 근거가 없다. 대신 **같은 성별·같은 연령대 안에서 상위 33%인가**로 정의한다. 근거가 감이 아니라 표본이 되고, 표본이 갱신되면 스크립트 한 번으로 임계값도 갱신된다.
...
읽는 법: 역삼각은 엉덩이가 가장 좁고(89.6) 어깨가 가장 넓다(35.9). 삼각은 정반대(98.5 / 34.6). 모래시계는 허리가 가장 가늘고(70.8, 엉덩이 대비 0.76), 둥근은 허리가 엉덩이에 붙는다(91.3 vs 98.2, 0.93). **이름과 실측이 일치한다** — v2에서는 확인할 수 없던 것이다. 남성 값은 `body_shape_thresholds.json`의 `centroids_cm`에 있다.
...
## 3. 세로축 — 3차원 비율 (현재 입력 없음, 기본값 `balanced`)
...
**그래서 지금은 모두 `balanced`로 두고 세로축 규칙을 적용하지 않는 것이 안전하다.** 비율을 모른 채 "크롭+하이웨스트"를 일괄 추천하면 상체가 짧은 사용자에게는 상체를 더 짧게 만드는 정반대 처방이 나간다.

> 🔎 **사이즈코리아 8차 원본에는 이 축의 재료가 있다.** `앉은키`, `샅높이`, `몸통수직길이`, `엉덩이수직길이`, `목뒤높이` 컬럼이 그대로 들어 있다 — 사이즈코리아가 "긴 지체 / 짧은 지체"를 판정할 때 쓰는 항목(몸통수직길이 대비 샅높이)이 바로 이것이다. 임계값 산출은 지금도 가능하고, 막혀 있는 건 **사용자 입력 쪽**(pose 파이프라인)이다. 이 축을 열려면 임계값이 아니라 입력부터 만든다.
