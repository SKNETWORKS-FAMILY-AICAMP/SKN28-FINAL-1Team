...
sequenceDiagram
    ...
    A->>V: 사진 2장 + 기본정보 전송
    V-->>A: 치수 12개 (실측 평균 10.8초)
    Note over A: 12개 중 하나라도 누락·비수치면 실패<br/>기본 모델 값으로 대체하지 않는다
    A->>A: 비율 2개 계산 · DB 저장(기존 14개 교체) · succeeded
    F->>A: GET /body/photos/{tx_id}/ (폴링)
    A-->>F: status + measurement 14개
```

### 5.1 사진 경로는 VLM 단독이다 (기본 모델과 섞지 않는다)

**사진 경로는 무사진 모델을 호출하지 않는다.** `estimate_from_photos()`는 VLM 응답만
파싱해 반환하고, 12개 중 하나라도 없거나 숫자가 아니면 **기본 모델 값으로 채우지 않고
실패시킨다**. 저장 시에는 DB의 상세 14개를 통째로 교체한다.

```python
content = _call_vlm(prompt, front_image, side_image)
return _parse_prediction(content)   # 무사진 모델 호출 없음
```

따라서 두 경로는 **위아래로 쌓이는 구조가 아니라 나란히 있는 별개의 경로**다.
`estimate`를 먼저 호출하지 않고 `photos`만 호출해도 된다. 저장값은 **마지막에 성공한
추정 결과**이며, 사용자가 `PATCH`로 고친 값도 재추정하면 덮인다.

| 장점 | 대가 |
| --- | --- |
| `source: photo` 결과가 실제로 전부 사진 기반이라 출처가 섞이지 않는다 | VLM이 한 값만 빠뜨려도 전체가 실패한다 |
| 응답 형태가 무사진 경로와 같아 프론트 파서가 하나로 유지된다 | 하이브리드(§7.3)를 넣으려면 **병합 단계를 새로 추가**해야 한다 |

> ⚠️ 이전 판 문서에는 "기본 모델로 10개를 먼저 채운 뒤 VLM이 덮어쓴다"고 적혀 있었으나
> **현재 코드에는 그 단계가 없다** (`inference.py:640~641`, `body_inference.py:277~279`).
...
| 경로      | 모델                                      | 예측 부위                      |
| --------- | ----------------------------------------- | ------------------------------ |
| 사진 없음 | HistGradientBoosting **아티팩트 3개** (`hist_181` / `circumference` / `exact_lengths_v2`) | 치수 12개 + 비율 2개 계산 = 14개 |
| 사진 있음 | Kimi K2.5 (OpenRouter) **단독** | 치수 12개 + 비율 2개 계산 = 14개 (기본 모델 사용 안 함) |

> 무사진 3개는 알고리즘(`HistGradientBoostingRegressor` + `MultiOutputRegressor`)도 입력
> (성별·키·몸무게)도 같다. **아티팩트가 셋인 이유는 학습 데이터가 나뉘어 있어서다** —
> 직접측정 172행 / 178행 / 3D 4,485행이며 두 계열의 `subject_id` 교집합은 0명이다.
...
4. 부위별로 나은 쪽을 채택하는 하이브리드는 7개 기준 2.167cm로 두 방식 단독보다 낫다.
   다만 **현재 사진 경로는 VLM 단독이라 "덮어쓸 부위를 제한"할 대상이 없다**(§5.1).
   적용하려면 무사진 결과와 VLM 결과를 **병합하는 단계를 새로 추가**하고, 이긴 부위만
   VLM 값으로 교체한 뒤 `apply_ratios()`로 비율을 다시 계산해야 한다.
   **아직 적용하지 않았다.** 위 비교가 **둘레 7개만** 커버해 **길이 5개는 어느 쪽이 나은지
   판단할 근거가 없기 때문이다.** 길이 정답 데이터 확보가 선행 과제다.
...
| 응답 스키마 통일                                                  | ④ ↔ ⑥ 완전 동일                     |
| 사진 경로 HTTP 종단                                               | 여·남 각 1건 성공 (당시 10개 계약 기준. 현재 계약은 14개) |
...
| 사진 vs 무사진 A/B              | 완료                 |
| HTTP 종단 검증                  | 완료                 |
| 하이브리드 적용 (부위별 채택)   | 미적용 — 길이 5개 판단 근거 부재 (§7.3) |
