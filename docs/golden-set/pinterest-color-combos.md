# Pinterest 색조합 참고 (Real-world Color Combinations)

> 작성일: 2026-08-07
> 출처: Pinterest 검색 "옷 색 조합" (https://kr.pinterest.com/search/pins/?q=옷 색 조합)
> 목적: 12 pants × 17 tops 시스템에 Pinterest에서 자주 등장하는 색조합을 cross-reference로 추가
...
### 향후 확장 아이디어
- 17 tops × 12 pants = 204 조합 (현재 17장 = top별 시트)
- Pinterest가 강조한 6대 카테고리별 top3 outfit 추천 (총 18장)
...
**추구미 (PursuitMe)**: Pinterest 카테고리 필터 추가 (Korean aesthetic / Chocomint / Tie Formal / Color clash / Tonal)

**채팅 에이전트**: "Pinterest 인기 조합" → 위 6대 인사이트 자동 매칭
```
사용자: "Pinterest 같은 감성 코디 추천해줘"
에이전트: → "Blush Pink t-shirt + Cream pants" (Korean Aesthetic)
       → "Teal t-shirt + Brown pants" (Chocomint)
       → "Olive t-shirt + Khaki pants" (Tonal)
```

**옷장 RAG**: Pinterest 카테고리 → 4 카테고리(Warm/Cool/Neutral/Muted) 자동 매핑 → 룰 적용
...
_이 문서는 데이터 기반 색조합 시스템에 Pinterest 시장 트렌드를 cross-reference로 추가한 자료입니다._
