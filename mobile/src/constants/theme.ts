 ...
 *
 * 밝은 면 위의 ink() 와 짝을 이룬다 — 면이 어두운 자리에서 ink() 를 쓰면 글자가
 * 배경에 묻힌다. 코코아(#4A3A30) 기준 실측: 1.0 = 10.8:1, 0.65 = 5.6:1(AA 여유),
 * 0.55 = 4.5:1(AA 하한). **0.55 아래로 내리지 말 것.**
 */
...
export const Editorial = {
  ...
  /**
   * 내비게이션 면 — 웹의 하단 탭바·좌측 사이드바.
   *
   * 앱에서 **유일하게 어두운 면**이다. 본문(오트)과 10.1:1 로 갈려서 테두리 없이도
   * 경계가 또렷하다 — 그래서 두 바의 테두리를 뺐다.
   * cta 와 값이 같지만 토큰을 나눠 둔 건 나중에 한 쪽만 조정하기 위해서다.
   * 이 위에 얹는 글자·아이콘은 반드시 onNav() 를 쓴다(ink 는 안 보인다).
   */
  nav: '#4A3A30',
