...
export const Editorial = {
  ...
  /**
   * 내비게이션 면 — 웹의 하단 탭바·좌측 사이드바. 본문(오트)보다 밝되 더 따뜻하다.
   *
   * ⚠️ **밝기만 올리면 흰색으로 읽힌다.** 처음엔 오트의 색상·채도를 유지한 채 밝기만
   * 올렸는데(#FCFAF8), 흰색에 가까워질수록 채널 차가 눌려 온기(R-B)가 8 → 4 로 반이
   * 됐다. 수치상 '같은 색상'이어도 눈에는 그냥 흰 판이었다.
   * 그래서 **온기를 오히려 키우면서**(R-B 8 → 11) 밝기를 올린다.
   *
   * 본문과의 대비는 1.02:1 뿐이다 — 경계를 만드는 건 색이 아니라 함께 두는
   * 헤어라인 테두리와 탭바 그림자다. 둘 중 하나라도 빼면 바가 본문에 녹는다.
   */
  nav: '#FDF9F2',
