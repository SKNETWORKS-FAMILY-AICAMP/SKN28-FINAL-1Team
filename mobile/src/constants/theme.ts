...
export const Editorial = {
  ...
  /**
   * 내비게이션 면 — 웹의 하단 탭바·좌측 사이드바. **면 색 규칙의 유일한 예외**다.
   *
   * 본문 면은 오트 한 색인데 내비게이션만 순백으로 한 단계 밝다(둘의 대비 1.07:1,
   * 밝기차 6.7포인트). 아주 미묘하지만 두 면이 맞닿아 있고 헤어라인 테두리가 함께 있어
   * 경계가 읽힌다 — '떠 있는 조작 면'과 '읽는 면'을 가르는 것이 목적이다.
   * 네이티브는 시스템 탭바를 쓰므로 여기 해당하지 않는다.
   */
  chrome: '#FFFFFF',
