...
export const Editorial = {
   ...
   *
   * 본문(오트)에서 밝기만 1.6포인트 올린 값이다(색상 38° 그대로라 같은 계열로 읽힌다).
   * ⚠️ 두 면의 대비는 **1.03:1** — 색만으로는 사실상 구분되지 않는다.
   *    경계를 만드는 건 함께 두는 **헤어라인 테두리와 그림자**다. 둘 중 하나라도 빼면
   *    바가 본문에 녹아 사라진다(코코아였을 땐 대비가 10:1이라 테두리 없이도 됐다).
   */
  nav: '#FCFAF8',
