 ...
 *   - STEP3  "완료"          → PATCH /body/detail/ 로 수정한 상세 항목 저장          (saveDetail)
 *
...
/**
 * 서버가 추정하는 상세 항목. 키 이름은 백엔드 BODY_DETAIL_FIELDS 와 1:1 이라
...
export const MEASURE_KEYS = BODY_MEASURES.map((spec) => spec.key) as readonly BodyMeasureKey[];
...
/**
 * 추정 결과의 상세 항목을 화면용 Measurement 로 바꾼다.
 * 하나라도 비어 있으면 null — 빈 칸을 0 이나 임의값으로 메우면 사용자는 그게
 * 추정 실패인지 진짜 0 인지 구분할 수 없다. (서버는 성공 시 상세 항목을 모두 채워 보낸다)
 */
...
const POLL_MAX_ATTEMPTS = 150; // 2초 × 150 = 최대 약 5분
const POLL_MAX_CONSECUTIVE_ERRORS = 3;

// 화면이 먼저 대기를 끝내도 서버의 진행 중 작업은 유지된다. 새 업로드 대신 같은 작업을 이어서 기다린다.
...
/** 추정 결과 → 화면 상태. 상세 항목을 못 읽으면 null (호출부가 실패로 처리한다). */
...
export const measureStore = {
  ...
  async startPhotoMeasurement(): Promise<void> {
    ...
    try {
      ...
      const estimation = await pollTransaction(transactionId);

      if (!estimation) {
        setState({
          status: 'error',
          result: null,
          error: '측정이 아직 끝나지 않았어요. 잠시 후 다시 시도하면 같은 측정을 이어서 확인해요.',
        });
        return;
      }
  ...
  /**
   * STEP3 "완료" — 수정한 상세 항목을 서버에 저장(PATCH detail)한다.
