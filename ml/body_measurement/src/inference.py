...
⚠️ 사진 VLM에게 7개를 다 물어보지만, 정확도를 실제로 측정한 부위는
가슴·허리·엉덩이 3개뿐이다. ``data/splits/vlm_*_set.csv``와
``data/raw_test_data/summary_raw_test_data.csv`` 모두 이 3개까지만 정답을 갖고 있어
허벅지·종아리·팔뚝·어깨는 사진 기반 오차를 계산할 방법이 없다. 이 4개 값의 신뢰도는
근거 없이 가정하지 말 것.
...
# 단 정확도를 실제로 측정한 건 아래 CORE 3개뿐이다. vlm_*_set.csv와
# summary_raw_test_data.csv 모두 가슴·허리·엉덩이 정답까지만 갖고 있어서
# 나머지 4개는 사진 기반 오차를 계산할 방법이 없다.
PHOTO_TARGETS = TARGETS
# 검증된 3개. 응답에 이 값이 없으면 사진 추정이 실패한 것으로 본다.
...
def _parse_prediction(content: str) -> dict[str, float]:
    ...
    검증된 3개(가슴·허리·엉덩이)는 없으면 실패로 본다. 나머지 4개는 모델이
