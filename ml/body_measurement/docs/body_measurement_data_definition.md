...
모델 입력은 `gender`, `height(cm)`, `weight(kg)` 3개다. API/mobile의 성별은 `male/female`, ML 내부 학습 인코딩은 `M/F`다. 팔뚝둘레(`arm`), 허벅지둘레(`thigh`), 종아리둘레(`calf`)는 과거 잘못된 둘레 정의이므로 현재 출력으로 사용하지 않는다.
...
| `hip` | cm | `465. 엉덩이둘레` / 10 |
| `thigh_length` | cm | (`156. 샅높이` - `158. 무릎뼈가운데높이`) / 10 |
| `calf_length` | cm | (`158. 무릎뼈가운데높이` - `161. 가쪽복사높이`) / 10 |
| `torso_length` | cm | (`434. 어깨높이` - `438. 위앞엉덩뼈가시높이`) / 10 |
| `leg_length` | cm | (`439. 샅높이` - `440. 가쪽복사높이`) / 10 |
| `neck_length` | cm | (`140. 턱끝높이` - `143. 목앞높이`) / 10 |
...
| 상체 길이 | 어깨선/어깨높이 → 골반점 |
| 하체 길이 | 샅선/샅높이 → 복사뼈/발목 |
| 목길이 | 정면 기준 턱밑/턱끝 라인 → 목앞/쇄골 라인 |

SizeKorea 새 기준 참고 분포는 다음과 같다. 이 값은 해석·캡션용 참고 범위이며, 사용자가 직접 수정한 양수 값은 저장할 수 있게 둔다.
