...
> **복원 안내 (2026-08-21)**
> 이 파일은 `6c1f138`("신체측정 11개 지표 계약 정리", 2026-08-12)에서 `experiments/`·`reports/`와
> 함께 삭제됐던 것을 `ml/body_measurement/docs/` 아래로 복원한 것이다. 본문은 삭제 시점 그대로이며
> 내용을 수정하지 않았다. 따라서 아래 **경로 표기는 당시 기준**이고 현재와 다르다.
>
> | 본문 표기 | 현재 위치 |
> |---|---|
> | `src/benchmark.py` | `ml/body_measurement/legacy/src/benchmark.py` |
> | `src/compare_all_models.py` | `ml/body_measurement/legacy/src/compare_all_models.py` |
> | `experiments/tabular/**` | 저장소에서 삭제됨 — `git show 6c1f138^:<경로>`로 열람 |
> | `reports/*.csv`, `reports/*.png` | 저장소에서 삭제됨 — 위와 동일 |
> | `model_evaluation_summary.md` | `docs/body_measurement_model_evaluation.md`로 대체 |
>
> 표의 수치는 `6c1f138^`의 `experiments/tabular/_summaries/sizekorea-1000-v1/all_models_metrics.json`
> 및 각 모델 `metrics.json`에서 재계산해 7개 모델 전부 일치함을 확인했다(2026-08-21).
>
> ⚠️ 이 벤치마크는 **legacy 7둘레 기준**이다. 현행 서빙 모델의 항목별 MAE
> (`docs/body_measurement_model_evaluation.md`)와 **같은 표에 섞어 인용하지 않는다.**

> 이 문서는 `body-measurement` 기능 추가 전 산출한 **측정값 7개** 모델 비교
> 결과를 보존한 기록이다. 현재 API는 여기에 `neck_length`,
> `thigh_calf_ratio`, `torso_leg_ratio`를 추가해 10개 값을 반환하지만, 이
> 보고서의 표·차트에는 새 체형 지표가 포함되어 있지 않다. 10개 출력 검증은
> `model_evaluation_summary.md`와 `src/run_tabular_benchmark.py`를 기준으로 한다.
