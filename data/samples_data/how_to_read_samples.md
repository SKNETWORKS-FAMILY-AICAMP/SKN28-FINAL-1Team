...
`S3 skn28-cozy` 버킷에서 11개 데이터셋마다 대표 샘플 1~2개(가능하면 원본 파일 + 매칭 라벨/메타데이터 쌍)를 이 폴더(`_claude_downloaded_samples/<데이터셋 폴더>/`)에 받아뒀다. 여기 등장하는 포맷별로 파이썬에서 여는 법을 정리한다. `.wst.dev`/`.p`(pickle)/`.obj`+`.mtl`은 실제로 받은 샘플 파일을 직접 열어서 확인한 내용이고, `.jpg`/`.json`/`.csv`/`.xlsx`는 표준 포맷이라 별도 검증 없이 적었다.
...
각 파일 경로는 이 문서 기준 `<데이터셋 폴더>/<파일명>` 이다 (예: `01_의류통합데이터/01_sou_000254_001266_front_02top_01blouse_woman.jpg`).

## jpg (이미지)

```python
from PIL import Image
img = Image.open("01_의류통합데이터/01_sou_000254_001266_front_02top_01blouse_woman.jpg")
...
```python
import json
with open("03_패션상품_착용영상/0928015_B.json", encoding="utf-8") as f:
    data = json.load(f)
print(data.keys())
```
...
표준 csv는 그대로 `pandas.read_csv`. 다만 `04_HM_추천데이터/articles_sample_head.csv`는 원본 34.5MB 파일에서 앞부분만 바이트 단위로 잘라 받은 것이라 **마지막 줄이 중간에 끊겨 있다** — `on_bad_lines='skip'`으로 깨진 마지막 줄만 버리고 읽으면 된다 (확인 결과 658행 정상 로드됨).

```python
import pandas as pd
df = pd.read_csv("04_HM_추천데이터/articles_sample_head.csv", on_bad_lines="skip")

# 잘림 없는 표준 csv는 그냥:
df2 = pd.read_csv("12_FASCODE/Fashion-How24_sub1_val.csv")
```
...
```python
import pandas as pd
df = pd.read_excel("22_사이즈코리아/7차_인체치수조사_2015_치수데이터.xlsx", sheet_name=0)
...
```python
import pickle
with open("05_polyvore_outfits/typespaces.p", "rb") as f:
...
import trimesh
mesh = trimesh.load("10_한국인_3D스캐닝/3001M_BD_A.obj")
...
```python
with open("11_PoC_패션코디/ac_eval_t1.wst.dev", encoding="euc-kr", errors="replace") as f:
    lines = f.readlines()
print("".join(lines[:10]))
